...
The production server also has an **agent-readiness gate**: `bash scripts/agent-checks.sh` (against the same running prod server, also run in CI) asserts Markdown content negotiation, `Vary: Accept`, 406 for unsatisfiable Accept headers, real 404s with a recoverable body, the trust-anchor aliases and the Organization `contactPoint`. Add new page routes to its route loop too. The public score lives at https://is-agentic.com/scan/hagvall-labs.com (`npx is-agentic hagvall-labs.com`); the page-level rule it enforces that is easiest to regress is **content efficiency: readable main-content text must stay at least 5% of the HTML**, so keep the inlined CSS lean (see the CSS notes below).
...
- **Large `blur()` filters cost first-paint time.** The hero glows are radial-gradient divs (`glow-cobalt`/`glow-cobalt-soft` in `styles.css`), not `blur-3xl` elements.
- The devtools overlay must stay wrapped in `import.meta.env.DEV` in `__root.tsx`.
- **The inlined CSS is part of every HTML response, keep it lean.** `styles.css` imports Tailwind with `source(".")` so only `src/` is scanned for class names (otherwise utilities mentioned in AGENTS.md/README, like `blur-3xl`, get emitted). Shadows use the flat `@utility` classes `shadow-flat-sm/md/lg` and `pill-underline` (same values as Tailwind's `shadow-*`) and the header uses `.header-blur` instead of `backdrop-blur`: Tailwind's own versions drag in the whole `--tw-shadow`/`--tw-backdrop-*` variable chain (a dozen `@property` rules, ~3 KB) for a handful of elements. is-agentic's content-efficiency check (main text >= 5% of HTML) is what this protects.

## Site structure

The site is bilingual: **Swedish is the default** (`/`, `/maskera`, `/kontakt`, `/integritet`), English lives under `/en` (`/en`, `/en/maskera`, `/en/contact`, `/en/privacy`).
...
- `src/components/ContactPage.tsx`, `PrivacyPage.tsx`: shared bilingual contact and privacy pages. Privacy copy must stay synchronized with the actual Umami, Caddy log, retention and backup configuration
- `src/components/ui.tsx`: shared class recipes (buttons with explicit transition properties that only change color and shadow, never transform: buttons must not move on hover or press, inline links, the section `container`) and the small shared components `HeroGlows` (hero background) and `Cards` (icon/number card grids). Statically imported by `__root.tsx`, so it lives in the always-loaded root chunk and never costs a separate request; keep it small
...
- Files in `src/routes/`: thin route wrappers, each just `pageHead()` with per-language strings plus the page component
...
The symbol geometry lives as `BRAND_PATHS`/`BRAND_VIEWBOX` in `src/components/BrandSymbol.tsx` and is the single source for everything else: the rendered component, the data-URI favicon in `__root.tsx`, the files the site references in `public/` (`brand/hagvall-labs-symbol.svg` for the JSON-LD logo, `brand/og-image-sv.png`, `brand/og-image-en.png`, `favicon.svg`, `favicon.ico`, `apple-touch-icon.png`) and the full brand package in `brand/` (symbol, horizontal and stacked lockups, wordmark, social cards; each in color/ink/white, SVG + PNG). All of that is generated by `bun run build:brand` (`scripts/build-brand.ts`, needs ImageMagick and the macOS Avenir Next font, which it outlines to paths so no output depends on a font): rerun it after any change to the paths, colors or tagline, never edit the generated files by hand. `brand/README.md` carries the usage rules and color table. The wordmark is letterspaced uppercase Avenir Next Medium (0.15em tracking in the files), "HÄGVALL" in ink and "LABS" in cobalt; the site header renders it as system-font text (`uppercase tracking-[0.12em] font-medium` in `__root.tsx`). Social cards are per language: `public/brand/og-image-sv.png` and `og-image-en.png` (1200x630, symbol + wordmark + tagline in that language), referenced from `pageHead()` in `src/seo.ts` with `summary_large_image` (not from `__root.tsx`, where they would merge into duplicates). Brand colors are Tailwind theme tokens in `src/styles.css`: `cobalt` #1748d4 (primary/CTAs), `cobalt-deep` (hover), `ink` #20272d (text). The symbol is monochrome cobalt: the middle ribbon is a lighter tint of the same hue (#5b7ce4), there is no second brand color. The old symbol with a turquoise middle ribbon is retired; never reintroduce it.
...
- **Markdown content negotiation (acceptmarkdown.com) lives in the origin server too.** `serve-prod.ts` parses `Accept` properly (q-values, wildcards, q=0); `text/markdown` preferred over `text/html` gets a Markdown rendering of the same URL (`text/markdown; charset=utf-8`, `Vary: Accept, Accept-Encoding`), an Accept header that rules out both gets `406` with a plain-text list, everything else gets HTML with `Vary: Accept`. The Markdown is derived at request time from the server-rendered page by `scripts/html-to-markdown.ts` (`<main>` + `<footer>`, front matter from title/description/canonical/hreflang, then sitemap + llms.txt links), so it can never drift from the HTML and needs no extra copy: anything `aria-hidden`, `<svg>`, `<button>` is dropped, so mark purely visual elements that way. The Dockerfile copies both scripts. TanStack Start only renders documents for HTML Accept headers (anything else was a 500), so the server always asks the framework for HTML itself.
- **404s stay real 404s with a way out**: the router sets the status, `NotFound` in `__root.tsx` lists the existing pages plus `sitemap.xml` and `llms.txt`, in both languages, and the Markdown representation follows from that markup. The conventional English trust-anchor paths `/about`, `/contact` and `/privacy` are permanent redirects (the `aliases` map in `serve-prod.ts`) to `/en`, `/en/contact` and `/en/privacy`; they are not pages, so they stay out of the sitemap and `pagePaths`.
- Organization JSON-LD in `__root.tsx` carries `contactPoint` (email from `contactEmail`, `url` to the contact page, languages) and `address`; keep both when editing it, agents use them to verify the business.
...
- Maskera links out to its product site **maskera.dev**. The `maskera` npm package version is pinned as `maskeraVersion` in `src/seo.ts` (used in the SoftwareApplication JSON-LD); bump it when a new version is published
- **`public/.well-known/security.txt`** (RFC 9116) carries the security contact; `/security.txt` is a 301 alias in `serve-prod.ts`. Its `Expires` field must stay in the future: bump it yearly
...
- **Analytics uses self-hosted Umami.** The production-only tracker is proxied same-origin under `/analytics`; it uses no cookies, excludes query strings and hashes, respects Do Not Track, and records pageviews plus explicit `outbound-link-click` events. Never call `umami.identify()`, send visitor-provided event data, or enable replay/heatmaps without revisiting the privacy/footer copy, CSP and Lighthouse gates. Umami and PostgreSQL have no host ports; the bootstrap keeps replay/heatmaps disabled. `scripts/umami-maintenance.sh` deletes live analytics after 90 days, makes a daily backup and rotates backups after 7 days. The edge Caddy operational log masks IPs before disk, removes cookie/auth headers and retains logs for 30 days. Keep `PrivacyPage.tsx` accurate when any of this changes.
- **Public Lighthouse scores in the footer.** `.github/workflows/lighthouse.yml` runs `scripts/lighthouse-public.ts` daily and after every Deploy against the live site (all routes, mobile, four categories, median of 3 runs), uploads the HTML reports as an artifact, publishes them with an index page to GitHub Pages (https://joelhagvall.github.io/hagvall-labs/, Pages source must be "GitHub Actions") and commits `src/lighthouse-scores.json` (lowest median per category) only when a score changes. The footer in `__root.tsx` renders that file and links to the Pages reports and PageSpeed Insights, so the numbers are always a third-party measurement, never hand-edited: do not edit the JSON by hand. A score below the gates still fails the run, after the commit.
- Contact email is `work@joelhagvall.com` for now (temporary until `hello@hagvall-labs.com` is live). In code it lives ONLY in `contactEmail` in `src/seo.ts` (ContactPage, the footer and JSON-LD in `__root.tsx` and both contact route descriptions read it from there); `llms.txt` and `public/.well-known/security.txt` carry it as prose. Switch the constant and both files together.
