...
- **Public Lighthouse scores in the footer.** `.github/workflows/lighthouse.yml` runs `scripts/lighthouse-public.ts` daily and after every Deploy against the live site (all routes, mobile, four categories, median of 3 runs), uploads the HTML reports as an artifact and commits `src/lighthouse-scores.json` (lowest median per category) only when a score changes. The footer in `__root.tsx` renders that file and links to the run history and PageSpeed Insights, so the numbers are always a third-party measurement, never hand-edited: do not edit the JSON by hand. A score below the gates still fails the run, after the commit.
